...
the member name. */
    ...
    SELECT F.name ,CONCAT(M.firstname,' ',M.surname) as member_name ,SUM(F.membercost) AS membercost_total
    FROM 
      Members M
...
def select_all_tasks(conn):
    ...
    query1 = """
        SELECT M.firstname||M.surname, R.recommendedby
        FROM Members M
